...
const makeSnakeDirections = (enemies: ForestEnemy[]) =>
	Object.fromEntries(
		enemies
...
const cropDefs: Record<CropId, CropDef> = {
	turnip: {
		name: "Turnip",
		growDays: 2,
...
const standardCropIds: CropId[] = ["turnip", "carrot", "pumpkin", "corn"];
const allPlantableCropIds: CropId[] = [...standardCropIds, "coral_fruit"];

const animalDefs: Record<AnimalType, AnimalDef> = {
...
const purchasableAnimalTypes: AnimalType[] = ["cow", "sheep", "chicken"];
const rareCowVariantTypes: AnimalType[] = [
	"hippo",
	"unicorn",
	"mammoth",
	"slug",
...
const highValueChestAnimalTypes: AnimalType[] = [
	...purchasableAnimalTypes,
	...rareCowVariantTypes,
];
const isCowLikeAnimal = (type: AnimalType) =>
	type === "cow" || rareCowVariantTypes.includes(type);
...
const vendorShopMapByKey: Record<VendorKey, MapId> = {
...
const vendorByShopMap: Partial<Record<MapId, VendorKey>> = Object.fromEntries(
...
const doors: Record<MapId, Door[]> = {
	...
	house: [
		{ x: 7, y: 5, target: { map: "farm", x: 7, y: 8 }, label: "Exit House" },
	],
	barn: [],
	town: [
		{
	...
	seed_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 8, y: 8 }, label: "Exit" },
	],
	feed_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 15, y: 8 }, label: "Exit" },
	],
	animal_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 22, y: 8 }, label: "Exit" },
	],
	market_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 51, y: 8 }, label: "Exit" },
	],
	tool_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 33, y: 8 }, label: "Exit" },
	],
	clothing_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 39, y: 8 }, label: "Exit" },
	],
	cafe_shop: [
		{ x: 7, y: 7, target: { map: "town", x: 45, y: 8 }, label: "Exit" },
	],
	forest: [],
	cave: [],
};
...
const petOptions: PetEmoji[] = ["🐈", "🐈‍⬛", "🐕", "🐩"]; // cat, black cat, dog, poodle
...
const isForestWalkableTile = (tile: string) => tile === "," || tile === "+";
const isForestBlockedTile = (tile: string) => tile === "T" || tile === "G";
const isCaveWalkableTile = (tile: string) =>
	tile === "=" || tile === ")" || tile === "+" || tile === "/";
const isCaveBlockedTile = (tile: string) =>
	tile === "<" || tile === ">" || tile === "*";
const forestSideOrder: ForestSide[] = ["top", "right", "bottom", "left"];
const oppositeForestSide = (side: ForestSide): ForestSide =>
	side === "left"
		? "right"
		: side === "right"
			? "left"
...
const generateForestState = (cfg?: ForestGenConfig): ForestGenerationResult => {
	const level = Math.max(1, cfg?.level ?? 1);
	if (level % 5 === 0) {
...
const buildCaveRubble = (layout: string[]): Record<string, string> => {
...
const generateCaveState = (cfg?: ForestGenConfig): CaveGenerationResult => {
	const level = Math.max(1, cfg?.level ?? 1);
	const entranceSide = cfg?.entranceSide ?? "left";
