...
export type PlayerInteractContext = {
	...
	playWater: () => void;
	playSnakeSound: () => void;
	setWaterRefillTile: Dispatch<
		SetStateAction<{ map: MapId; x: number; y: number } | null>
	>;
...
export const runInteract = (ctx: PlayerInteractContext, dir: Dir): void => {
	...
	if (player.map === "farm" && farmWeedObstacles[farmTargetKey]) {
		...
		setFarmWeedObstacles((prev: Record<string, boolean>) => ({
			...prev,
			[farmTargetKey]: false,
		}));
		playHoe();
	...
	if (player.map === "farm" && farmForestBlockers[blockerKey]) {
		...
		if (!trySpendStamina(getSmashAxeActionCost(smashAxeLevel))) return;
		setFarmForestBlockers((prev: Record<string, boolean>) => ({
			...prev,
			[blockerKey]: false,
		}));
		playHoe();
	...
	if (player.map === "farm" && baseTile === "," && !plots[plotKey]) {
		...
		let hoedCount = 0;
		const hasObstacleBetweenPlayerAndTarget = (
			targetX: number,
			targetY: number,
		) => {
			if (dir === "right") {
